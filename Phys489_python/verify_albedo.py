...
verify_albedo.py
================
Verification of the multifractal surface albedo cascade against Universal
Multifractal (UM) theory (Schertzer & Lovejoy 1987).

Usage
-----
1. In Unity Play Mode, press Z to export albedo snapshots.
   Files land in Phys489_python/snapshots-albedo/albedo_NNN.csv
2. Set H_TARGET and C1_TARGET below to match the Unity inspector values.
3. Run:  python verify_albedo.py

Outputs
-------
  Albedo_Verification_Panel.png  —  2×2 diagnostic figure
...
# =============================================================================
# PARAMETERS — match Unity inspector values
# =============================================================================
H_TARGET     = 0.66
C1_TARGET    = 0.12
ALPHA_TARGET = 1.79   # Lévy index: 1.79 = Lovejoy empirical, 2.0 = log-normal

# Theoretical spectral slope: β = 1 + 2H − K(2)
# General UM: K(2) = C1/(α−1) * (2^α − 2)
...
SNAPSHOTS_FOLDER = "snapshots-albedo"
ALBEDO_PATTERN   = os.path.join(SNAPSHOTS_FOLDER, "albedo_*.csv")
...
def run_verification():
    files = sorted(glob.glob(ALBEDO_PATTERN))
    if not files:
        print(f"No albedo snapshots found matching: {ALBEDO_PATTERN}")
        print("Press Z in Unity Play Mode to export albedo fields first.")
        return

    print(f"Found {len(files)} albedo snapshot(s).")
    ...
    #
    #    equatorial_belt=True: restrict sampling to ±45° latitude to avoid
    #    polar bias where a pixel lag r covers ~r·sin(θ)→0 physical distance.
    #
    #    H_eff is derived from the measured beta rather than the nominal H_TARGET.
    #    The discrete cascade + Hermite smoothstep systematically steepen the
    #    spectrum, so H_eff > H_TARGET.  Using H_eff in K(q) = q·H − ζ(q)
    #    removes the linear bias seen in panel C.
    #    General UM: β = 1 + 2H − K(2),  so H_eff = (β_avg − 1 + K2_THEORY) / 2
    # -------------------------------------------------------------------------
    ...
    sf   = structure_functions(field_for_moments, q_values, r_values, equatorial_belt=True)
    zeta = fit_zeta(sf, r_values, q_values)

    H_eff = (beta_avg - 1.0 + K2_THEORY) / 2.0
    Kq    = q_arr * H_eff - zeta
    ...
    # -------------------------------------------------------------------------
    # 4. Independence check: cross-correlation with the cloud field
    #    AlbedoNoiseOrigin=(500,500,500) decorrelates albedo from cloud space
    #    [0,100].  This check verifies that decorrelation is effective.
    # -------------------------------------------------------------------------
    cloud_folder = "snapshots-cloud"
    cloud_files  = sorted(glob.glob(os.path.join(cloud_folder, "snapshot_*.csv")))
    if cloud_files:
        cloud_data = load_field(cloud_files[0])
        min_size   = min(field_for_moments.size, cloud_data.size)
        r_cross    = np.corrcoef(
            field_for_moments.ravel()[:min_size],
            cloud_data.ravel()[:min_size]
        )[0, 1]
        print(f"\n--- Independence ---")
        print(f"  Cross-correlation (albedo vs cloud) = {r_cross:.4f}   (target: ~0)")

    # -------------------------------------------------------------------------
    # 5. Verification panel figure (2×2)
    # -------------------------------------------------------------------------
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        f"Albedo Cascade Verification  |  H={H_TARGET}, C₁={C1_TARGET}, α={ALPHA_TARGET}  |  "
        f"N={len(files)} realization(s)",
        fontsize=13
    )

    # Panel A: raw field (Mollweide-style imshow of the equirectangular grid)
    ax = axes[0, 0]
    im = ax.imshow(np.log1p(field_for_moments), cmap="terrain", aspect="auto", origin="upper")
    plt.colorbar(im, ax=ax, label="log(1 + albedoIntensity)")
    ax.set_title("A — Raw Albedo Field (log scale)")
    ...
    ax.set_xlabel("log(albedoIntensity)")
    ...
    plt.tight_layout()
    out_path = "Albedo_Verification_Panel.png"
